...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        if idx % 500 == 0:
            print(f"已处理 {idx}/{len(df)} 条评论的情感分析")
        ...
        if len(lang_texts) > 0:
            ...
            if len(lang_texts) > 0:
                ...
                for i in range(topic_modeler.n_topics):
                    df[f'{lang}_topic_{i+1}_prob'] = 0.0  # 初始化列
                    for j, prob in enumerate(topic_distributions[:, i]):
                        if j < len(df[df['language'] == lang]):
                            # 找到对应语言的行并设置概率
                            lang_indices = df[df['language'] == lang].index
                            if j < len(lang_indices):
                                df.at[lang_indices[j], f'{lang}_topic_{i+1}_prob'] = prob
