...
def generate_wordcloud(texts, language, output_path):
    """
    生成词云
    """
    # 合并所有文本
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        if idx % 20 == 0:
            print(f"已处理 {idx+1}/{len(df)} 条评论的情感分析")
    ...
    plot_sentiment_distribution(df, sentiment_dist_path)
    ...
    for lang in df['language'].unique():
        ...
        if len(lang_texts) > 0:
            ...
            generate_wordcloud(lang_texts, lang, wordcloud_path)
